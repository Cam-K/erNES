...
int decodeAndExecute(CPU* cpu, Bus* bus){
  ...
  switch(bus->cpu->opcode){
    ...
    case 0x86:
      ...
      break;
    case 0x88:
      cyclesCompleted = dey(bus);
      break;
    case 0x8a:
      cyclesCompleted = txa(bus);
      break;
    case 0x8c:
    ...
    case 0xa6:
      ...
      break;
    case 0xa8:
      cyclesCompleted = tay(bus);
      break;
    case 0xa9:
      cyclesCompleted = lda(cpu, bus, immediate);
      break;
    case 0xaa:
      cyclesCompleted = tax(bus);
      break;
    case 0xac:
...
int clc(Bus* bus){
  bus->cpu->pf = clearBit(bus->cpu->pf, C);
...
int cli(Bus* bus){
  bus->cpu->pf = clearBit(bus->cpu->pf, I);
  // dummy read
  readBus(bus, bus->cpu->pc);
  bus->cpu->pc++;
  return 2;
...
int sta(CPU* cpu, Bus* bus, AddrMode mode){
  ...
  switch(mode){
    ...
    case zeroPage:
      ...
      break;
    case indirectX:
      operand = readBus(bus, ++cpu->pc);
      readBus(bus, operand);
      pointer = operand + cpu->x;
      bus->cpu->addressL = readBus(bus, pointer);
      bus->cpu->addressH = readBus(bus, pointer += 1);
      break;
    case indirectY:
      pointer = readBus(bus, ++cpu->pc);
      bus->cpu->addressL = readBus(bus, pointer);
      bus->cpu->addressH = readBus(bus, pointer += 1);
      beforeValue = bus->cpu->addressL;
      bus->cpu->addressL += cpu->y;
      readBus(bus, (((uint16_t)bus->cpu->addressH) << 8) + bus->cpu->addressL);
      // carry out value
      if(beforeValue > bus->cpu->addressL){
        bus->cpu->addressH++;
      }
...
int stx(CPU* cpu, Bus* bus, AddrMode mode){
  ...
  if(mode == absolute){
    ...
    bus->cpu->addressL = readBus(bus, ++cpu->pc) + cpu->y;
...
int txa(Bus* bus){
  bus->cpu->a = bus->cpu->x;
  checkNFlag(bus->cpu, bus->cpu->a);
  checkZFlag(bus->cpu, bus->cpu->a);
  readBus(bus, bus->cpu->pc);
...
void addressModeDecodeWrite(uint8_t value, CPU* cpu, Bus* bus, AddrMode mode){
  ...
  switch(mode){
    ...
    case absoluteX:
      ...
      return;
    case absoluteY:
      writeBus(bus, (((uint16_t)bus->cpu->addressH) << 8) + (uint16_t)bus->cpu->addressL + cpu->y, value);
      return;
    case accumulator:
    ...
    case zeroPageY:
      ...
      return;
    case indirectX:
      writeBus(bus, (((uint16_t)bus->cpu->addressH) << 8) + bus->cpu->addressL, value);
      return;
    case indirectY:
      writeBus(bus, (((uint16_t)bus->cpu->addressH) << 8) + bus->cpu->addressL, value);
      return;
...
uint8_t addressModeDecode(CPU* cpu, Bus* bus, AddrMode mode){
  ...
  switch(mode){
    ...
    case absolute:
      ...
      return readBus(bus, (((uint16_t)bus->cpu->addressH) << 8) + (uint16_t)bus->cpu->addressL);
    ...
    case zeroPageX:
      ...
      bus->cpu->addressL = zeroPageAddr + cpu->x;
      return readBus(bus, bus->cpu->addressL);
    ...
    case indirectX:
      bus->cpu->addressL = readBus(bus, (uint8_t)(cpu->x + readBus(bus, ++cpu->pc))); 
      bus->cpu->addressH = readBus(bus, (uint8_t)(cpu->x + readBus(bus, cpu->pc) + 1)); 

      return readBus(bus, (((uint16_t)bus->cpu->addressH) << 8) | bus->cpu->addressL);
      
    case indirectY:
      zeroPageAddr = readBus(bus, ++cpu->pc); 
      bus->cpu->addressL = readBus(bus, zeroPageAddr);
      ...
      return readBus(bus, ((((uint16_t) bus->cpu->addressH) << 8) + (bus->cpu->addressL + cpu->y)));
    default:
      halt(cpu);
      return 0;
